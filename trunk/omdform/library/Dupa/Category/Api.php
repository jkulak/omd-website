...
class Dupa_Category_Api
{
	...
	public function getCategory( $id )
	{
		...
		if( $id > 0 )
		{
    		$query = 'SELECT id, name, added, addedby, updated, updatedby FROM '.self::DB_TABLE.' WHERE id = ?';
	...
	public function setCategory( Dupa_Category_Container $category )
	{
		...
		if( $category )
		{
		    ...
		    try
		    {
    		    $result = $this->_db->update( self::DB_TABLE, $data, 'id = ' . $category->getId() );
		    }
			catch( Zend_Db_Exception $e )
    		{
	...
	public function getCategoriesCnt()
	{
		...
		{
		    throw new Dupa_Exception( 'Error getting categories count', Dupa_Exception::ERROR_DB );
		}
	}
}
